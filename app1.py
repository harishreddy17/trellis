...
def generate_image_from_text(text_prompt: str) -> List[Image.Image]:
    """
    Generate images from a text prompt using a pretrained model.
    
    Args:
        text_prompt (str): The text description to generate an image from.
        
    Returns:
        List[Image.Image]: A list of generated images.
    """
    try:
        # Use your pretrained model to generate the image(s)
        generated_image = pipe(prompt=prompt).images[0]
        
        # Ensure that we are dealing with a list of PIL Image objects
        generated_image.save("generated_image_koala.png")
...
def image_to_3d(
    ...
    if is_prompt:
        ...
        if image is None:
            return {}, "Error generating image from text."
...
with gr.Blocks(delete_cache=(600, 600)) as demo:
    ...
    * Enter a text prompt or upload an image to create a 3D asset. Click "Generate" to proceed.
    ...
    generate_btn.click(
        ...
        image_to_3d,
        inputs=[image_prompt, text_prompt, gr.State(True), seed, ss_guidance_strength, ss_sampling_steps, slat_guidance_strength, slat_sampling_steps],
