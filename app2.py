...
with gr.Blocks(delete_cache=(600, 600)) as demo:
    ...
    * Enter a text prompt or upload an image to create a 3D asset. Click "Generate" to proceed.
    ...
    # Button handlers and logic
    generate_btn.click(
        get_seed,
        inputs=[randomize_seed, seed],
        outputs=[seed],
    ).then(
        lambda text_prompt: determine_state(text_prompt),  # Evaluate the state based on text_prompt
        inputs=[text_prompt],  # Use the text_prompt value
        outputs=[is_prompt],  # Update the state dynamically
    ).then(
        image_to_3d,
        inputs=[
            image_prompt,  # Pass the Gradio component itself
            text_prompt,  # Pass the Gradio component directly
            is_prompt,  # Dynamically evaluated state (True/False)
            seed,
